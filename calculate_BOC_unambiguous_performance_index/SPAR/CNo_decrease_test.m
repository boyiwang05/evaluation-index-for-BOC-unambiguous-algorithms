...
%%
%%%相关器间隔
BW=30*1.023e6;d=0.1*Tc;Dz=3.671e-4;ALFA=1;
Pulse_width=0.18;%BOC(10,5)相关器间隔
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N_BW=10000;
...
%         N_CNo_sampling=mod(     n   ,    floor(  n_loop/length(C_N0_dB)    )    ) ;
        ...
        Q_NoisePower=N0*f_sample;
         ...
         [s_ref1_E,s_ref2_E,t_E]=yt_BOCs_SPAR_function(c,t_ref_begin+d/2,t_ref_end+d/2,Tc,fs,f_sample,Pulse_width);
         s_ref1_E=s_ref1_E/sqrt(mean(s_ref1_E.*conj(s_ref1_E)));
         s_ref2_E=s_ref2_E/sqrt(mean(s_ref2_E.*conj(s_ref2_E)));
            
         [s_ref1_L,s_ref2_L,t_L]=yt_BOCs_SPAR_function(c,t_ref_begin-d/2,t_ref_end-d/2,Tc,fs,f_sample,Pulse_width);
         s_ref1_L=s_ref1_L/sqrt(mean(s_ref1_L.*conj(s_ref1_L)));
         s_ref2_L=s_ref2_L/sqrt(mean(s_ref2_L.*conj(s_ref2_L)));
       ...
       %%
        ...
        %% 鉴别器     
        DiscrimOut  = (IE1^2+QE1^2)-(IL1^2+QL1^2)-ALFA*(IE2^2+QE2^2)+ALFA*(IL2^2+QL2^2);%鉴别器2
...
% end
savefile='CNo_desrease_test_SPAR_method_BOC_10_5_30M_01Tc.mat';
save(savefile);
...
figure;plot(Trackerror/3e8/Tc,'Linewidth',2);grid on;
xlabel('Time (ms)');
ylabel('Code tracking error (chips)');
saveas(gcf,'CNo_desrease_test_SPAR_method_BOC_10_5_30M_01Tc');

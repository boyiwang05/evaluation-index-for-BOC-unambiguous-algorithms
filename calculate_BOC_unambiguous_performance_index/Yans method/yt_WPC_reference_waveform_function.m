function [s1,s2,t]=yt_WPC_reference_waveform_function(c,t_begin,t_end,Tc,fs,fsample)
%c是一个码周期的扩频码序列
%t_begin起始时间
%t_end终止时间
%Tc码片宽度
...
% s2(index_2_temp)=1;
% s2=sqrt(k/(k-2))*s2.*code.*Subcarrier_BOC;
s2=s2.*code.*Subcarrier_BOC;
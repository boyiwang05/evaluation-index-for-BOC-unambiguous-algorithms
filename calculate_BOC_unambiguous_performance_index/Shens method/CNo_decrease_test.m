...
%         N_CNo_sampling=mod(     n   ,    floor(  n_loop/length(C_N0_dB)    )    ) ;
        ...
        Q_NoisePower=N0*f_sample;
         ...
         [s_ref1_E,s_ref2_E,t_E]=radioEng_2015_paper_function(c,t_ref_begin+d/2,t_ref_end+d/2,Tc,fs,f_sample,K);
         [s_ref1_L,s_ref2_L,t_L]=radioEng_2015_paper_function(c,t_ref_begin-d/2,t_ref_end-d/2,Tc,fs,f_sample,K);
       ...
       %%
        ...
        %% 鉴别器     
         DiscrimOut= ((IE1+IE2)^2-((1+K)/(1-K)*(IE1-IE2))^2)+((QE1+QE2)^2-((1+K)/(1-K)*(QE1-QE2))^2)-((IL1+IL2)^2-((1+K)/(1-K)*(IL1-IL2))^2)-((QL1+QL2)^2-((1+K)/(1-K)*(QL1-QL2))^2);
...
% end
savefile='CNo_desrease_test_Shens_method_BOC_10_5_30M_01Tc.mat';
save(savefile);
...
saveas(gcf,'CNo_desrease_test_Shens_method_BOC_10_5_30M_01Tc');

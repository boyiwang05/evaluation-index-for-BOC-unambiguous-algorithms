...
%%
%%%相关器间隔
BW=36*1.023e6;d=0.03*Tc;Kass=0.3;Dz=2.595e-4;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
N_BW=10000;
...
for m=1:Num_false_acquisition
    for k=1:N_C
        ...
        for n=1:n_loop
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%
         [s_ref1_E,s_ref2_E,t_E]=yaozheng_PCF_function(c,t_ref_begin+d/2,t_ref_end+d/2,Tc,fs,f_sample,Kass);
         [s_ref1_L,s_ref2_L,t_L]=yaozheng_PCF_function(c,t_ref_begin-d/2,t_ref_end-d/2,Tc,fs,f_sample,Kass);
         [s_ref1_P,s_ref2_P,t_P]=yaozheng_PCF_function(c,t_ref_begin,t_ref_end,Tc,fs,f_sample,Kass);    
        ...
        DiscrimOut  = ((abs(IE1)+abs(IE2)-abs(IE1+IE2))^2+(abs(QE1)+abs(QE2)-abs(QE1+QE2))^2)-((abs(IL1)+abs(IL2)-abs(IL1+IL2))^2+(abs(QL1)+abs(QL2)-abs(QL1+QL2))^2);%鉴别函数
...
savefile='false_lock_index_PUDLL_BOC_10_5_30M_01Tc_45dBHz.mat';
save(savefile);
...
figure;plot(t_ref_begin_scan/Tc,TrackErrSTD,'b','Linewidth',2);grid on;
hold on;plot(t_ref_begin_scan/Tc,ones(1,length(TrackErrSTD))*d/Tc/6,'r','LineWidth',2);
xlabel('false acuiqisition results (chips)');
ylabel('code tracking error std (chips)');
saveas(gcf,'false_lock_index_PUDLL_BOC_10_5_30M_01Tc_45dBHz');
